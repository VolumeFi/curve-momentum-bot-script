...
const STOP_LOSS = 4;
const BOT_NAME = 'curveMomentum';
const WithdrawType = {
    SUCCESS: 1,
    EXPIRED: 2, // For Limit order, Stop loss bot type. Not for this Momentum bot
    REMAINING: 3 // For DCA bot type. Not for this Momentum bot
};
...
async function getNewBlocks(fromBlock) {
    ...
    console.log("deposited events", deposited_events.length);
    ...
    if (withdrawn_events.length !== 0) {
        ...
        for (const withdrawn_event of withdrawn_events) {
            ...
            try {
                ...
                if (botInfo && withdrawn_event.returnValues["withdraw_type"] !== 'CANCEL') {
                    axios.get(TELEGRAM_ALERT_API, {
                        params: {
                            depositor: botInfo["depositor"],
                            kind: WithdrawType.SUCCESS, // Success case
                            tokenName: tokenName,
                            botType: BOT_NAME, // Momentum bot type
                        },
                    });
                }
            } catch (error) {
...
async function processDeposit(deposit) {
    ...
    let profit_taking = Number(deposit.deposit_price) + Number(deposit.deposit_price) * Number(deposit.profit_taking) / Number(DENOMINATOR);
    let stop_loss = Number(deposit.deposit_price) - Number(deposit.deposit_price) * Number(deposit.stop_loss) / Number(DENOMINATOR);

    if (Number(price) > profit_taking) {
        return { "deposit_id": Number(deposit.deposit_id), "withdraw_type": PROFIT_TAKING };
    } else if (Number(price) < stop_loss) {
